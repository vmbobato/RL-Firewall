...
def train_sarsa_agent(
    ...
    """
    Train a tabular SARSA agent on the FirewallEnv and log results.

    Training:
      - num_episodes passes through the dataset with epsilon-greedy policy.
      - On-policy updates: Q(s,a) <- Q(s,a) + alpha [r + gamma Q(s',a') - Q(s,a)]
        ...
        while True:
            ...
            if terminated:
                ...
                next_action = epsilon_greedy_action(
                    q_table, next_state_key, n_actions, epsilon, rng
                )
                next_q = q_table.get((next_state_key, next_action), 0.0)
    ...
    print("\n=== SARSA Greedy Evaluation (epsilon=0) ===")
    obs, info = env.reset()
    state_key = get_state_key(obs)
    ...
    while True:
        ...
        max_q = max(q_values)
        best_actions = [a for a, q in enumerate(q_values) if q == max_q]
        eval_action = int(np.random.choice(best_actions))

        next_obs, reward, terminated, info = env.step(eval_action)
        next_state_key = get_state_key(next_obs)
        eval_reward += reward
        ...
        if label == 1 and eval_action == 1:
            tp += 1
        elif label == 1 and eval_action == 0:
            fn += 1
        elif label == 0 and eval_action == 1:
            fp += 1
        elif label == 0 and eval_action == 0:
            tn += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SARSA firewall agent")
    parser.add_argument(
        "--episodes",
        type=int,
        default=500,
