...
def evaluate_q_learning_agent(
    ...
    """
    Train and evaluate a Tabular Q-Learning agent on the FirewallEnv for num_episodes.
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            if done:
                ...
                # Use the best action regardless of the action chosen for the next state
                temporal_diff_target = reward + gamma * np.max(Q[next_state])
            ...
            # Choose the next epsilon-greedy action and update the action
            if not done:
                if np.random.rand() < epsilon:
                    action = env.action_space.sample()
                else:
    ...
    print("\n=== Running Q-Learning Greedy Evaluation (epsilon = 0) ===")
    ...
    print("\n=== Q-Learning Greedy Evaluation Results ===")
